...
var MultiSelect = (function (_super) {
                    ...
                    if (this.liCollections) {
                        if (this.checkTextLength() && !this.allowFiltering && (e.keyCode !== 8)) {
                            ...
                            if (this.allowFiltering) {
                                ...
                                liCollections = this.list.querySelectorAll('li.' + dropDownBaseClasses.li);
                                var activeElement = Search(this.inputElement.value, liCollections, 'StartsWith', this.ignoreCase);
                                if (activeElement && activeElement.item !== null) {
                                    this.addListFocus(activeElement.item);
                                    this.list.scrollTop =
    ...
    MultiSelect.prototype.addListSelection = function (element) {
        var className = this.hideSelectedItem ?
            HIDE_LIST :
            dropDownBaseClasses.selected;
        if (this.enabled && this.isValidLI(element) && !element.classList.contains(dropDownBaseClasses.hover)) {
            addClass([element], className);
